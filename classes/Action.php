 ...
 */
class WSC_Classes_Action extends WSC_Classes_FrontController {
    ...
    public function getActions($ajax = false) {
        $this->actions = array();
        $action = WSC_Classes_Tools::getValue('action');
        $nonce = WSC_Classes_Tools::getValue('wsc_nonce');
        ...
        if (!isset(self::$config)) {
            $config_file = _WSC_ROOT_DIR_ . '/config.json';
        ...
        if ($ajax) {
            check_ajax_referer(_WSC_NONCE_ID_, 'wsc_nonce');
        } else {
            check_admin_referer($action, 'wsc_nonce');
        }
        /* add the actions in WP */
        foreach ($this->actions as $actions) {
            WSC_Classes_ObjController::getClass($actions['class'])->action();
        }
    }
