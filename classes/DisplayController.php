 ...
 */
class WSC_Classes_DisplayController {
    ...
    public static function loadMedia($uri = '', $media = 'all') {
        ...
        if (strpos($uri, '//') === false) {
            $name = strtolower($uri);
            if (file_exists(_WSC_THEME_DIR_ . 'css/' . $name . '.css')) {
                $css_uri = _WSC_THEME_URL_ . 'css/' . $name . '.css?ver=' . WSC_VERSION_ID;
            }
            if (file_exists(_WSC_THEME_DIR_ . 'css/' . $name . '.scss')) {
                $css_uri = _WSC_THEME_URL_ . 'css/' . $name . '.scss?ver=' . WSC_VERSION_ID;
            }
            if (file_exists(_WSC_THEME_DIR_ . 'js/' . $name . '.js')) {
                $js_uri = _WSC_THEME_URL_ . 'js/' . $name . '.js?ver=' . WSC_VERSION_ID;
            }
        } else {
        ...
        if ($css_uri <> '') {
            if (!wp_style_is($name)) {
                if (did_action('wp_print_styles')) {
                    ...
                    wp_enqueue_style($name, $css_uri, null, WSC_VERSION_ID, $media);
        ...
        if ($js_uri <> '') {
            if (!wp_script_is($name)) {
                if (did_action('wp_print_scripts')) {
                    ...
                    wp_enqueue_script($name, $js_uri, null, WSC_VERSION_ID, true);
     ...
     *
     * @param $block
     * @param WSC_Classes_FrontController $view
     * @return null|string
     */
    public function getView($block, $view) {
        $output = null;

        if (file_exists(_WSC_THEME_DIR_ . $block . '.php')) {
            ob_start();
            include(_WSC_THEME_DIR_ . $block . '.php');
            $output .= ob_get_clean();
        }
