...
class WSC_Debug {
    ...
    public static function checkDebug() {
        ...
        if (self::getIsset('debug')) {
            if (self::getValue('debug') === 'on' && WSC_DEBUG) {
                error_reporting(E_ALL);
                @ini_set('display_errors', true);
                if (function_exists('register_shutdown_function')) {
                    register_shutdown_function(array(new WSC_Debug(), 'showDebug'));
                }
            }
        }
//        elseif (WSC_DEBUG) {
//            register_shutdown_function(array(new WSC_Debug(), 'showLoadingTime'));
//        }
    }
    ...
    public static function dump() {
        if (WSC_DEBUG) {
            ...
            $run_time = number_format(microtime(true) - WSC_REQUEST_TIME, 3);
    ...
    public static function showDebug() {
        WSC_Classes_Tools::setHeader('html');
        $errors = error_get_last();
        WSC_Debug::dump($errors);
...
defined('WSC_DEBUG') || define('WSC_DEBUG', false);
WSC_Debug::checkDebug();
